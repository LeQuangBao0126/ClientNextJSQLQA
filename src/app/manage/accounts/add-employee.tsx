...
export default function AddEmployee() {
    ...
    const onSubmit = async  (values :CreateEmployeeAccountBodyType) => {
        if (file) {
            ...
            values = { ...values, avatar: uploadResult.payload.data }
        }
        // add thành công là invalidate queries 
        const res = await addAccountMutation.mutateAsync(values)
        form.reset()
        setOpen(false )
    }
    return (
        <Dialog onOpenChange={setOpen} open={open}>
            ...
            <DialogContent className='sm:max-w-[600px] max-h-screen overflow-auto'>
                ...
                <Form {...form}     >
                    <form noValidate className='grid auto-rows-max items-start gap-4 md:gap-8' id='add-employee-form'
                        onSubmit={form.handleSubmit(onSubmit, (err) => {
                            console.log("Lỗi khi thêm account nhan vien ", err)
                        })}
                        onReset={resetForm}
                    >
                ...
                <DialogFooter>
                    ...
                    <Button type='reset' className='bg-red-500' form='add-employee-form'>
                        Hủy
                    </Button>
                </DialogFooter>
            </DialogContent>
